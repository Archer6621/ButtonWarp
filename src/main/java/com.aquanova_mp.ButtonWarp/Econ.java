package com.aquanova_mp.ButtonWarp;
...
public class Econ {
     ...
     * @param sourceName The name of the Player/Bank that will receive the money
     * @param amount The amount that will be charged
     * @return True if the transaction was successful
     */
    public static boolean charge(Player player, String sourceName, double amount) {
        ...
        if (sourceName.equalsIgnoreCase("server")) {
            return true;
        }

        if (sourceName.startsWith("bank:")) {
            //Send money to a bank account
            economy.bankDeposit(sourceName.substring(5), amount);
        } else {
            //Send money to a Player
            Player sourcePlayer = Bukkit.getPlayer(sourceName);
            if(sourcePlayer!=null)
                economy.depositPlayer(sourcePlayer, amount);
        }
     ...
     * @param sourceName The Player/Bank that will give the reward
     * @param amount The amount that will be rewarded
     */
    public static void reward(Player player, String sourceName, double amount) {
        ...
        if (!sourceName.equalsIgnoreCase("server")) {
            ...
            if (sourceName.startsWith("bank:")) {
                ...
                if (economy.bankHas(sourceName, amount).type != (ResponseType.SUCCESS)) {
                    player.sendMessage(ButtonWarpMessages.sourceInsufficentFunds
                            .replace("<amount>", economy.format(amount)));
                    return;
                }

                economy.bankWithdraw(sourceName, amount);
            } else {
                //Cancel if the Source Player cannot afford the transaction
                Player sourcePlayer = Bukkit.getPlayer(sourceName);

                if(sourcePlayer!=null) {
                    if (!economy.has(sourcePlayer, amount)) {
                        player.sendMessage(ButtonWarpMessages.sourceInsufficentFunds
                                .replace("<amount>", economy.format(amount)));
                        return;
                    }

                    economy.withdrawPlayer(sourcePlayer, amount);
                }
            }
        }
